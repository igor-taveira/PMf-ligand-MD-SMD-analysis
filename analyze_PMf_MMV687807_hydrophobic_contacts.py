...
"""
Hydrophobic-contact analysis between MMV687807 (LIG) and the PMf membrane
during the staged PMf + MMV687807 trajectory.
...
EVENTS_OUT = os.path.join(
    OUTDIR,
    "hydrophobic_contacts_LIG_PMF_all_events.csv"
)

PER_FRAME_OUT = os.path.join(
    OUTDIR,
    "hydrophobic_contacts_LIG_PMF_per_frame.csv"
)

PAIR_OCC_OUT = os.path.join(
    OUTDIR,
    "hydrophobic_contacts_LIG_PMF_pair_occupancy.csv"
)

RESIDUE_OCC_OUT = os.path.join(
    OUTDIR,
    "hydrophobic_contacts_LIG_PMF_residue_occupancy.csv"
)

LIPID_OCC_OUT = os.path.join(
    OUTDIR,
    "hydrophobic_contacts_LIG_PMF_lipid_type_occupancy.csv"
)

SUMMARY_OUT = os.path.join(
    OUTDIR,
    "hydrophobic_contacts_LIG_PMF_summary.txt"
)

COUNT_PNG = os.path.join(
    OUTDIR,
    "hydrophobic_contacts_LIG_PMF_per_frame.png"
)

PAIR_OCC_PNG = os.path.join(
    OUTDIR,
    "hydrophobic_contacts_LIG_PMF_top10_pair_occupancy.png"
)

RESIDUE_OCC_PNG = os.path.join(
    OUTDIR,
    "hydrophobic_contacts_LIG_PMF_top10_residue_occupancy.png"
)

LIPID_OCC_PNG = os.path.join(
    OUTDIR,
    "hydrophobic_contacts_LIG_PMF_lipid_type_occupancy.png"
)
...
# Hydrophobic contact cutoff in Å
CONTACT_CUTOFF_A = 4.5

# Analyze every frame
STEP = 1

# Number of top contacts/residues shown in figures
TOP_N = 10
...
# PMf membrane components in this system
MEMBRANE_RESNAMES = [
    "CER160",
    "BMAN",
    "DYPC",
...
def is_hydrophobic_atom(atom):
    ...
    When PSF partial charges are available, strongly polar/charged
    C/S atoms are excluded using abs(charge) > 0.5.
    ...
    if not (likely_carbon or likely_sulfur):
        return False

    # Exclude strongly charged/polar C/S atoms where possible
    try:
...
def save_per_frame_plot(df):
    ...
    plt.xlabel("Time (ns)")
    plt.ylabel("Hydrophobic contacts")
    plt.title("LIG–PMf hydrophobic contacts")
...
def save_pair_plot(df):
    ...
    values = top["occupancy_percent"].to_numpy()
    y = np.arange(len(top))
    ...
    plt.title("Top LIG–PMf hydrophobic contacts")
...
def save_residue_plot(df):
    ...
    plt.xlabel("Occupancy (%)")
    plt.title("PMf residues forming hydrophobic contacts with LIG")

    plt.xlim(
        0,
        max(
            100.0,
            top["occupancy_percent"].max() * 1.10
        )
    )
...
def save_lipid_plot(df):
    ...
    plt.xlabel("Occupancy (%)")
    plt.title("Hydrophobic-contact occupancy by PMf component")

    plt.xlim(
        0,
        max(
            100.0,
            plot_df["occupancy_percent"].max() * 1.10
        )
    )
...
def main():
    print("=" * 80)
    print("LIG <-> PMf HYDROPHOBIC CONTACT ANALYSIS")
    print("PMf + MMV687807 staged trajectory")
    print("=" * 80)
    ...
    for ts in u.trajectory[::STEP]:
        ...
        # Find hydrophobic atom pairs within cutoff
        ...
        frame_rows.append({
            "frame": frame,
            "time_ns": time_ns,
            "hydrophobic_contacts": n_contacts,
            "has_hydrophobic_contact": int(
                n_contacts > 0
            )
        })
        ...
        if (
            ...
            for pair, distance in zip(
                ...
                lig_local = int(pair[0])
                mem_local = int(pair[1])
                ...
                event_rows.append({
                    "frame": frame,
                    "time_ns": time_ns,
                    "ligand_idx": lig_atom.index,
                    "membrane_idx": mem_atom.index,
                    "ligand_atom": atom_label(lig_atom),
                    "membrane_atom": atom_label(mem_atom),
                    "membrane_residue": residue_label(mem_atom),
                    "membrane_resname": mem_atom.resname,
    ...
    #
    # Occupancy =
    # frames containing pair / sampled frames * 100
    # ========================================================
    ...
    #
    # Percentage of frames in which ligand has at least
    ...
    with open(
        ...
        f.write(
            "PMf + MMV687807 staged trajectory\n"
        )
